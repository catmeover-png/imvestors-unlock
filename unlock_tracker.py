...
def find_ws(ss, title: str):
    """Worksheet lookup that ignores case and surrounding spaces.

    Google Sheets treats sheet names as unique case-insensitively, so an
    existing 'labels' or 'Labels ' blocks creating 'Labels', while gspread's
    ss.worksheet() only matches the exact title.
    """
    try:
